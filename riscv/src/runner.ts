...
export class Runner {
    ...
    //     const incrementProgramCounter = true
    ...
    //     const valueOne = this.registers.get(command.sourceRegisterOneToken)
    //     const valueTwo = this.registers.get(command.sourceRegisterTwoToken)
    //     const result = valueOne + valueTwo
    //     this.registers.set(command.destinationRegisterToken, result)
    // }
    ...
    //             break
    //         default:
    //             throw new Error(`invalid store command '${command.nameToken}'`)
    //     }
